...
class GridDataset(Dataset):
    ...
    def get_grid(self):
        ...
        for name in self.ids:
            ...
            for win_points in windows_list:
                ...
                patch_size = swin_tiny_unet.DATA.IMG_SIZE
                # grid_img = zoom(grid_img, (patch_size / h, patch_size / w, 1), order=3)
                # grid_mask = zoom(grid_mask, (patch_size / h, patch_size / w, 1), order=3)
                grid_img = cv2.resize(grid_img, (patch_size, patch_size),
                                      interpolation=cv2.INTER_AREA)
                grid_mask = cv2.resize(grid_mask, (patch_size, patch_size),
                                       interpolation=cv2.INTER_AREA)
    ...
    def __getitem__(self, idx):
        ...
        img = self.preprocess(img,
                              self.scale,
                              is_mask=False,
                              is_transforms=self.is_transforms)
...
class BasicDataset(Dataset):
    ...
    def preprocess(cls, pil_img, scale, is_mask, is_transforms):
        ...
        # # swin
        # patch_size = 224
        # pil_img = zoom(pil_img, (patch_size / h, patch_size / w, 1), order=3)
        # if h>7000:
        #     scale=0.1
...
class PimgDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        assert len(
            pimg_file
        ) == 1, f'Either no image or multiple images found for the ID {name}: {pimg_file}'
