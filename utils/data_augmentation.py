...
def mosaic_aug(images_dir, masks_dir):
    ...
    # get all imgs name
    all_name_list = [
        splitext(file)[0] for file in listdir(images_path)
        if (file.endswith('.png') or file.endswith('.jpg'))
        and not file.endswith('_mosaic.png')
    ]
    # print(all_name_list)
    ...
    for i in range(no_aug):
        ...
        # print(img_id)
        imgs, masks = mosaic_load(img_id, all_name_list, images_path,
                                  masks_path)
        img1, img2, img3, img4 = imgs
        mask1, mask2, mask3, mask4 = masks

        # find mini weight and hight: img.shape=(hight,weight,channel)
        hight_min = min(imgs[0].shape[0], imgs[1].shape[0], imgs[2].shape[0],
                        imgs[3].shape[0])
        weight_min = min(imgs[0].shape[1], imgs[1].shape[1], imgs[2].shape[1],
                         imgs[3].shape[1])
        # print('weight_min,hight_min: ',weight_min,hight_min)

        # random mosaic point
        x_mosaic = np.random.randint(int(weight_min * 0.3),
                                     int(weight_min * 0.7))
        y_mosaic = np.random.randint(int(hight_min * 0.3),
                                     int(hight_min * 0.7))
        ...
        # x轴方向拼接
        img11_mosaic = img1[y1_mosaic - y_mosaic:y1_mosaic,
                            x1_mosaic - x_mosaic:x1_mosaic]
        img12_mosaic = img2[y2_mosaic - y_mosaic:y2_mosaic,
                            x_mosaic:weight_min]
        ...
        mask11_mosaic = mask1[y1_mosaic - y_mosaic:y1_mosaic,
                              x1_mosaic - x_mosaic:x1_mosaic]
        mask12_mosaic = mask2[y2_mosaic - y_mosaic:y2_mosaic,
                              x_mosaic:weight_min]
        ...
        mask21_mosaic = mask3[y_mosaic:hight_min,
                              x3_mosaic - x_mosaic:x3_mosaic]
...
def tensor_img_mask_aug(img, mask):
    '''
    augmentation img on line - flip, rotation
    //transforms img data augmentation will not change the tensor size 
    //if w,h are not same, 
    //output will crop where there is redundant data
    //will fill up with black pixs where there is Empty position
    ...
    # rotation
    degrees = int(random.randint(0, 3) * 90)
    if degrees>0:
        augs_list.append(transforms.RandomRotation(degrees=(degrees, degrees)))

    # if np.random.rand() > 0.5:
    #     degrees = int(random.randint(1, 3) * 90)
    #     augs_list.append(transforms.RandomRotation(degrees=(degrees, degrees)))
...
if __name__ == '__main__':
    ...
    # img = torch.rand((1, 3, 1024, 512))
    img_name='test_sw.png'
    ...
    # # test numpy.rot90
    # print('test numpy.rot90')
    # rot90_test=np.rot90(img,k)
    # print('rot90_test shape', rot90_test.shape)
    # cv2.imwrite('../testData/rot90_test.png',rot90_test)

    # test transforms.RandomRotation
    print('test transforms.RandomRotation')
    import torch
    ...
    degrees=int(90*k)
    rotate_img = transforms.RandomRotation(degrees=(degrees, degrees))(img_tensor)
    print('rotate_img size', rotate_img.size())
    rotate_test = rotate_img.squeeze().permute(1, 2, 0).numpy()
    print('rotate_test type', type(rotate_test))
    print('rotate_test shape', rotate_test.shape)
    rotate_test_name='rotate_test.png'
    cv2.imwrite(img_dir+rotate_test_name,rotate_test)
    # re rotation
    rerotate_img = transforms.RandomRotation(degrees=(-degrees, -degrees))(rotate_img)
    print('rerotate_img size', rerotate_img.size())
    rerotate_test = rerotate_img.squeeze().permute(1, 2, 0).numpy()
    print('rerotate_test type', type(rerotate_test))
    print('rerotate_test shape', rerotate_test.shape)
    rerotate_test_name='rerotate_test.png'
    cv2.imwrite(img_dir+rerotate_test_name,rerotate_test)